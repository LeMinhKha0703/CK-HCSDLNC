...
const MCQDetails: React.FC = () => {
  ...
  const totalStudents = exam?.submissions.length ?? 0;
      ...
      <div className="bg-[#f3f4f6] rounded-xl p-8 mb-8 shadow-sm">
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-500 mt-1">
            Number of students who answered each question correctly (out of {totalStudents})
          </p>
        </div>
        {chartData.length === 0 ? (
          <div className="text-center py-10 text-gray-400 italic">No submission data available for chart yet.</div>
        ) : (
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 20, right: 12, left: 0, bottom: 12 }}>
                ...
                <Tooltip
                  ...
                  formatter={(value: number) => [`${value} students`, 'Correct']}
                />
                <Bar dataKey="correctAnswers" name="Correct Answers" fill="#1a4cd2" radius={[8, 8, 0, 0]} barSize={20} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        )}
